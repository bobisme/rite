...
kitty --title "botbus-screenshot" -e bash -c "./target/release/botbus ui; read" &
KITTY_PID=$!
...
hyprctl dispatch focuswindow "title:botbus-screenshot" >/dev/null
...
GEOMETRY=$(hyprctl clients -j | jq -r '.[] | select(.title == "botbus-screenshot") | "\(.at[0]),\(.at[1]) \(.size[0])x\(.size[1])"')
...
grim -g "$GEOMETRY" /tmp/botbus-tui-auto.png
...
pngquant --force --output "$OUTPUT" /tmp/botbus-tui-auto.png
