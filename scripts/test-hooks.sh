...
RITE="${RITE:-cargo run --release --bin rite --}"
RITE_DATA_DIR=$(mktemp -d)
export RITE_DATA_DIR
export RITE_AGENT="${RITE_AGENT:-test-agent}"
...
echo "data_dir: $RITE_DATA_DIR"
echo "agent:    $RITE_AGENT"
echo

# Ensure data dir is initialized
$RITE init >/dev/null 2>&1 || true

# --- Add a hook ---
echo "1. Add hook"
OUTPUT=$($RITE hooks add \
    --channel test-hook \
...
$RITE send test-hook "trigger message" >/dev/null
AUDIT=$(cat "$RITE_DATA_DIR/hooks_audit.jsonl")
...
$RITE send test-hook "second trigger" >/dev/null
AUDIT2=$(tail -1 "$RITE_DATA_DIR/hooks_audit.jsonl")
...
$RITE claim "agent://test-dev" -m "working" >/dev/null
pass "claim created"
echo

# --- Send message (hook should NOT fire — claim held) ---
echo "8. Send message (claim held → hook should not fire)"
$RITE send test-hook "trigger with claim" >/dev/null
AUDIT3=$(tail -1 "$RITE_DATA_DIR/hooks_audit.jsonl")
...
$RITE send test-hook "trigger after release" >/dev/null
AUDIT4=$(tail -1 "$RITE_DATA_DIR/hooks_audit.jsonl")
...
$RITE claim "agent://test-dev" -m "blocking" >/dev/null
TEST2=$($RITE hooks test "$HOOK_ID")
echo "$TEST2"
...
# --- Remove hook ---
echo "12. Remove hook"
$RITE hooks remove "$HOOK_ID" >/dev/null
LIST2=$($RITE hooks list)
...
AUDIT_COUNT=$(wc -l < "$RITE_DATA_DIR/hooks_audit.jsonl")
